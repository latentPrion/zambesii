...
{
	ptrlessListC<Block>::Iterator		it;
	Block					*currBlock;
	Allocation				*ret;
...
{
	ptrlessListC<Block>::Iterator		it, prev;
	ubit8					*allocEnd;
...
void *Heap::malloc(size_t sz, void *allocatedBy, utf8Char *desc)
{
	//size_t				origSz=sz;
	error_t					err;
	ptrlessListC<Chunk>::Iterator		it;
	Chunk					*currChunk;
	Allocation				*ret;
	...
	for (sbit8 pass=1; pass <= 2; pass++)
	{
		...
		chunkList.insert(
			currChunk,
			ptrlessListC<Chunk>::OP_FLAGS_UNLOCKED);
	};
...
void Heap::free(void *_p, void *freedBy)
{
	...
	ptrlessListC<Chunk>::Iterator		chunkIt;
...
void Heap::dumpChunks(ubit32 flags)
{
	...
	printf(NOTICE"dumpChunks: %u chunks, chunk size %u, dumping chunks.\n",
		chunkList.getNItems(ptrlessListC<Chunk>::OP_FLAGS_UNLOCKED),
		chunkSize);
...
{
	ptrlessListC<Allocation>::Iterator	it;

	printf(NOTICE"dumpAllocs: %u allocs, spanning %u chunks\n",
		allocationList.getNItems(ptrlessListC<Allocation>::OP_FLAGS_UNLOCKED),
		chunkList.getNItems(ptrlessListC<Chunk>::OP_FLAGS_UNLOCKED));
...
void Heap::dumpBlocks(ubit32 flags)
{
	...
	{
		Chunk					*currChunk;
		ptrlessListC<Block>::Iterator		it2;
...
{
	ptrlessListC<Chunk>::Iterator		iChunks;
...
error_t Heap::checkBlocks(void)
{
	...
	public:
		...
		localAutoUnlockerC(ptrlessListC<Chunk> *list)
		:
		list(list)
		{}
	...
	for (; iChunks != chunkList.end(); ++iChunks)
	{
		ptrlessListC<Block>::Iterator	iBlocks, iBlocksPrev;
		Chunk				*currChunk;
		ubit8				*currChunkEnd;
...
error_t Heap::checkAllocations(void)
{
	...
	ptrlessListC<Chunk>::Iterator		iChunks;
	ptrlessListC<Allocation>::Iterator	iAllocs;
	...
	for (; iAllocs != allocationList.end(); ++iAllocs, nAllocsInList++)
	{
		...
		if (!chunkList.find(
			alloc->parent, ptrlessListC<Chunk>::OP_FLAGS_UNLOCKED)
