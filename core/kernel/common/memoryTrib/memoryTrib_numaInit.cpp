...
error_t memoryTribC::initialize2(void)
{
	...
	{
		__kprintf(NOTICE MEMTRIB"Failed to init chipset FWStream.\n");
		return ret;
	};
	...
	if (ret != ERROR_SUCCESS)
	{
		__kprintf(NOTICE MEMTRIB"Failed to init firmware FWStream.\n");
		return ret;
	};
	...
	if (ret != ERROR_SUCCESS)
	{
		__kprintf(NOTICE MEMTRIB"Chipset provides no Mem Info Riv.");
	...
	if (ret != ERROR_SUCCESS)
	{
		__kprintf(NOTICE MEMTRIB"Failed to init Mem Info Rivulet.\n");
		return ret;
	};
	...
	if (memConfig != __KNULL && memConfig->memSize > 0)
	{
		...
		ret = numaTrib.spawnStream(CHIPSET_MEMORY_NUMA_SHBANKID);
		if (ret != ERROR_SUCCESS)
		{
			__kprintf(ERROR MEMTRIB"Failed to spawn shbank.\n");
			goto parseMemoryMap;
		};
...
void memoryTribC::init2_spawnNumaStreams(chipsetNumaMapS *map)
{
	...
	for (uarch_t i=0; i<map->nMemEntries; i++)
	{
		...
		if (numaTrib.getStream(map->memEntries[i].bankId)) {
			continue;
		};

		// Else allocate a new stream.
		ret = numaTrib.spawnStream(map->memEntries[i].bankId);
		if (ret != ERROR_SUCCESS)
		{
			__kprintf(ERROR MEMTRIB"Failed to spawn NUMA stream for "
				"detected bank %d.\n",
